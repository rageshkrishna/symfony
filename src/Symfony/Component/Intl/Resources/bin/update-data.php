...
use Symfony\Component\Intl\Data\Generator\DataGeneratorInterface;
...
$txtDir = $baseDir.'/txt';
$jsonDir = $baseDir.'/'.Intl::JSON;
$phpDir = $baseDir.'/'.Intl::PHP;
$resDir = $baseDir.'/'.Intl::RB_V2;

$targetDirs = array($jsonDir, $phpDir, $resDir);
$workingDirs = array($jsonDir, $phpDir, $txtDir, $resDir);

$config->addBundleWriter($txtDir, new TextBundleWriter());
$config->addBundleWriter($jsonDir, new JsonBundleWriter());
$config->addBundleWriter($phpDir, new PhpBundleWriter());
...
$generator = new LocaleDataGenerator(
    Intl::LOCALE_DIR,
    new LanguageDataProvider($resDir.'/'.Intl::LANGUAGE_DIR, $reader),
    new ScriptDataProvider($resDir.'/'.Intl::SCRIPT_DIR, $reader),
    new RegionDataProvider($resDir.'/'.Intl::REGION_DIR, $reader)
);
